        ...
        if ob.type == "MESH":
            ...
            if len(ob.modifiers):
                self.layout.operator(
                    "object.forced_modifier_apply", icon_value=common.kiss_icon()
                )
...
class CNV_UL_modifier_selector(common.CNV_UL_generic_selector):
    ...
    use_filter_viewport_visible = bpy.props.BoolProperty(
        ...
        update=_gen_visible_update(
            "use_filter_viewport_visible", "use_filter_renderer_visible"
        ),
    )
    use_filter_renderer_visible = bpy.props.BoolProperty(
        ...
        update=_gen_visible_update(
            "use_filter_renderer_visible", "use_filter_viewport_visible"
        ),
    )
    ...
    def filter_items(self, context, data, propname):
        flt_flags, flt_neworder = super(CNV_UL_modifier_selector, self).filter_items(
        ...
        if (
            self.use_filter_viewport_visible
            or self.use_filter_renderer_visible
            or getattr(self, "force_values")
        ):
            ...
            for index, item in enumerate(items):
                ...
                if self.use_filter_viewport_visible and item.filter0:
                    ...
                    not self.use_filter_viewport_visible
                    and not self.use_filter_renderer_visible
                ):
                    pass
                else:
                    flt_flags[index] |= ~out_flag
...
class CNV_OT_forced_modifier_apply(bpy.types.Operator):
    ...
    is_preserve_shape_key_values = bpy.props.BoolProperty(
        name="Preserve Shape Key Values",
        default=True,
        description="Ensure shape key values are not changed",
    )
    ...
    apply_viewport_visible = bpy.props.BoolProperty(
        name="Apply Viewport-Visible Modifiers", default=False
    )
    apply_renderer_visible = bpy.props.BoolProperty(
        name="Apply Renderer-Visible Modifiers", default=False
    )

    initial_progress = bpy.props.FloatProperty(
        name="Progress", default=-1, options={"HIDDEN", "SKIP_SAVE"}
    )
    ...
    def invoke(self, context, event):
        ...
        for index, mod in enumerate(ob.modifiers):
            ...
            #    break
            icon = "MOD_%s" % mod.type.replace("DECIMATE", "DECIM").replace(
                "SOFT_BODY", "SOFT"
            ).replace("PARTICLE_SYSTEM", "PARTICLES").replace(
                "_SPLIT", "SPLIT"
            ).replace("_PROJECT", "PROJECT").replace("_DEFORM", "DEFORM").replace(
                "_SIMULATION", "SIM"
            ).replace("_EDIT", "").replace("_MIX", "").replace(
                "_PROXIMITY", ""
            ).replace("_PAINT", "PAINT")
            icon = compat.icon(icon)
    ...
    def draw(self, context):
        ...
        if compat.IS_LEGACY:
            self.layout.prop(
                prefs,
                "custom_normal_blend",
                icon=compat.icon("SNAP_NORMAL"),
                slider=True,
            )
        self.layout.prop(
            self,
            "is_preserve_shape_key_values",
            icon=compat.icon("SHAPEKEY_DATA"),
            slider=True,
        )
        ...
        self.layout.template_list(
            "CNV_UL_modifier_selector", "", self, "is_applies", self, "active_modifier"
        )
    ...
    def execute(self, context):
        ...
        if self.apply_viewport_visible or self.apply_renderer_visible:
            for index, mod in enumerate(ob.modifiers):
                ...
                new_prop.index = index
                new_prop.value = (
                    self.apply_viewport_visible and mod.show_viewport
                ) or (self.apply_renderer_visible and mod.show_render)
        ...
        if not is_any:
            self.report(
                type={"INFO"},
                message="適用対象のモディファイアがないため、キャンセルします",
            )
        ...
        for index, mod in enumerate(copy_modifiers):
            # if index >= 32: # luvoid : can only apply 32 modifiers at once.
            #    break
            if self.is_applies[index].value and (
                mod.type != "ARMATURE" or not compat.IS_LEGACY
            ):
                ...
                try:
                    ...
                    self.report(
                        type={"ERROR", "WARNING"},
                        message=f_tip_(
                            "Could not apply '{type}' modifier \"{name}\"",
                            type=mod.type,
                            name=mod.name,
                        ),
                    )
            ...
            context.window_manager.progress_update(
                progress_start + (progress + mod_progress / mod_count) / progress_count
            )
        ...
        for index, mod in enumerate(copy_modifiers):
            ...
            #    break
            if self.is_applies[index].value and (
                mod.type == "ARMATURE" and compat.IS_LEGACY
            ):
                try:
                    bpy.ops.object.modifier_apply(override, modifier=mod.name)
                except Exception as e:
        ...
        if is_shaped:
            for deforms in new_shape_deforms:
                if len(me.vertices) != len(deforms):
                    self.report(
                        type={"ERROR"},
                        message="ミラー等が原因で頂点数が変わっているためシェイプキーを格納できません、中止するのでCtrl+Z等で元に戻し修正してください。",
                    )
                    context.window_manager.progress_update(progress_start + 1)
                    if did_start_progress:
                        context.window_manager.progress_end()
        ...
        if arm_ob:
            for i, loop in enumerate(me.loops):
                ...
                try:
                    custom_rot = mathutils.Vector((0.0, 0.0, 1.0)).rotation_difference(
                        custom_normals[i]
                    )
                except:
                    continue
