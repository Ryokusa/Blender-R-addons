...
i18n_contexts = {
    identifer: getattr(bpy.app.translations.contexts, identifer)
    for identifer in bpy.app.translations.contexts_C_to_py.values()
}
"""
i18n_contexts = {
    default_real        = None                ,
...
def get_best_locale_match(locale: str, available=handled_locales) -> str:
    ...
    for locale_tag in DICT.keys():
        language, country, variant, language_country, language_variant = (
            bpy.app.translations.locale_explode(locale_tag)
        )
...
def message_to_csv_line(msg, add_sources=False):
    ...
    sources = (
        f'FROM <{" & ".join(get_message_source_file(src) for src in msg.sources)}>'
        if add_sources
        else ""
    )
...
def messages_to_csv(msgs, reports, lang=get_locale(), only_missing=True):
    ...
    messages = sorted(
        (
            (key, msg, get_message_source_file(msg.sources[0]))
...
class CNV_OT_dump_py_messages(bpy.types.Operator):
    ...
    language = bpy.props.EnumProperty(
        items=items, name="Language", default=get_locale()
    )
    ...
    def execute(self, context):
        ...
        reports = extract_messages._gen_reports(
            extract_messages._gen_check_ctxt(bl_i18n_settings)
            if self.do_checks
            else None
        )
        ...
        txt_data = messages_to_csv(
            msgs, reports, lang=self.language, only_missing=self.only_missing
        )
        ...
        self.report(
            type={"INFO"},
            message=f_tip_(
                "Strings have been dumped to {txt_name}. See text editor.",
                txt_name=txt_name,
            ),
        )
...
def register(__name__=__name__):
    ...
    # Since the add-on in Japanese we want to translate even when Blender's language is set to English.
        ...
        if (
            system.language in {"en_US", "DEFAULT"}
    ...
    for lang in os.listdir(translations_folder):
        ...
        for csv_file_name in os.listdir(lang_folder):
                ...
                try:
                    for line, row in enumerate(csv_reader):
                        ...
                        if row[0] not in i18n_contexts.values():
                            print_verbose(
                                2,
                                f'Unknown translation context "{row[0]}" on line {line}',
                            )
            ...
            print_verbose(
                1, f"-> Added {entry_count} translations from {csv_file_name}"
            )
            if is_check_duplicates:
                print_verbose(
                    1, f"-> Replaced {dupe_count} translations with {csv_file_name}"
                )
        print_verbose(
            0, f"-> Added {len(lang_dict) - orig_count} translations for {lang}"
        )
